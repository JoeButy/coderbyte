...
num_inputs = 5
op_list = ['/', '*', '+', '-']
op_perms = [p for p in iter.product(op_list, repeat=num_inputs-1)]
target = 237
...
# 		print eval_str
	...
	try:
		...
		tot = False
	if abs(tot - target) < .000001 and tot != target:
...
def int237(arr):
	...
	x_perms = [p for p in iter.permutations(arr, num_inputs)]
	total_hits = 0
...
for i in range(10):
	...
	results_matric.append([test_set, int237(test_set)])
print results_matric
print [79,0,1,3,5], int237([79,0,1,3,5])
print [237, 1, 2, 4, 8], int237([237, 1, 2, 4, 8])
print [3, 79, 237, 236, 1], int237([3, 79, 237, 236, 1])